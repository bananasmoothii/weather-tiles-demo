...
export default defineComponent({
  ...
  methods: {
    ...
    addCity() {
      let newCityName = this.newCityName.trim();
      if (newCityName === "") return;
      cities.splice(0, 0, nameToCity(newCityName));
      this.newCityName = "";
      document.getElementById("city-add-input")?.focus();
    },
  },
});
</script>
    ...
    >
      <TransitionGroup tag="ul" type="transition" class="sort-target" :name="drag ? undefined : 'fade'">
        <li v-for="(element, index) in cities" class="no-transition city-li" :key="element.randomId">
          ...
          <svg class="handle scale-125 cursor-move" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <title>drag</title>
            <path
